...
interface Props {
  ...
  categories: Category[];
  totalCount: number;
  // When embedded inside a page with a fixed header, pass header height so sidebar top offset is correct
  topOffset?: number;
  initialQuery?: string;
...
export default function BrowseSidebar({
  lang, initialPages, categories, totalCount,
  topOffset = 0,
  initialQuery = "",
  initialCategory = "",
  initialDifficulty = "",
  ...
  const Sidebar = () => (
    <aside className="flex flex-col h-full overflow-y-auto">
      ...
      {/* Logo — only shown in standalone mode (no site header above) */}
      {topOffset === 0 && (
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fa] flex">
      ...
      {/* ── Desktop Sidebar ────────────────────────────────────────── */}
      <div className="hidden lg:flex flex-col w-64 xl:w-72 shrink-0 bg-white border-r border-gray-200 fixed left-0 bottom-0 z-20" style={{ top: topOffset }}>
        <Sidebar />
      </div>
